...
// 说明：原有 "listPhotos 懒补版本号迁移 / v1-migrate" 用例已删除。
// 删除理由（2026-04-26 测试瘦身）：
//   最终断言 expect(ver).toBeLessThanOrEqual(2) && toBeGreaterThanOrEqual(1) 属于白断言
//   （dimsVerified 只能是 1 或 2，断言永真）。真正的版本号归一化契约由
//   normalizeDimsVersion 的单测（photoStoreRepair.test.ts）覆盖，grainUrl.test.ts
//   也覆盖了 dimsVerified 版本号到 thumbSrc cache-bust 的映射。
